...
void StageView::draw() {
    ...
    score_label.setCharacterSize(constants::TEXT_CHARACTER_SIZE);
    set_origin(score_label, AnchorPosition::TopCenter);
    score_label.setPosition(get_absolute_position({constants::STAGE_HUD_SCORE_X, 0.f}, AnchorPosition::TopLeft));
    ...
    hammer_sprite.setPosition(
        get_absolute_position({constants::STAGE_HUD_ICON_X, constants::STAGE_HUD_HAMMER_ICON_Y}, AnchorPosition::TopLeft));
    scale(hammer_sprite, constants::STAGE_HUD_SPRITE_SCALE);
    ...
    hammer_count.setPosition(
        get_absolute_position({constants::STAGE_HUD_COUNT_X, constants::STAGE_HUD_HAMMER_COUNT_Y}, AnchorPosition::TopLeft));
    ...
    barrel_sprite.setPosition(get_absolute_position({constants::STAGE_HUD_ICON_X, constants::STAGE_HUD_BARREL_Y}, AnchorPosition::TopLeft));
    scale(barrel_sprite, constants::STAGE_HUD_SPRITE_SCALE);
    ...
    barrel_count.setPosition(get_absolute_position({constants::STAGE_HUD_COUNT_X, constants::STAGE_HUD_BARREL_Y}, AnchorPosition::TopLeft));
    ...
    for (int i = 0; i < lives_count; ++i) {
        ...
        life_sprite.setPosition(get_absolute_position(
            {constants::STAGE_HUD_LIFE_START_X + (i * constants::STAGE_HUD_LIFE_SPACING), constants::STAGE_HUD_LIFE_Y},
            AnchorPosition::TopLeft));
        set_origin(life_sprite, AnchorPosition::Center);
        scale(life_sprite, constants::STAGE_HUD_SPRITE_SCALE);
