...
float DissolvingPlatform::slope_at(float x) const {
    return 0.f;
}

float DissolvingPlatform::surface_y_at(float x) const {
    return position.y;
}
...
void DissolvingPlatform::accept(EntityVisitor &visitor) {
    visitor.visit(*this);
}

const sf::RectangleShape& DissolvingPlatform::get_shape() const {
    return shape;
}
