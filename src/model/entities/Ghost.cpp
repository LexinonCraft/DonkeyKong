    ...
    switch (state) {
        case State::IdleOnPlatform:
            ...
            if (remaining_time <= 0.f) {
                ...
                remaining_time = constants::GHOST_MINIMUM_MOVING_DURATION + mod(stage.random_int(), constants::GHOST_MOVING_DURATION_STEPS) * (constants::GHOST_MAXIMUM_MOVING_DURATION - constants::GHOST_MINIMUM_MOVING_DURATION) / constants::GHOST_MOVING_DURATION_STEPS;
            }
            break;
        case State::MovingOnPlatform: {
            ...
            if (remaining_time <= 0.f) {
                ...
                remaining_time = constants::GHOST_MINIMUM_IDLE_DURATION + mod(stage.random_int(), constants::GHOST_IDLE_DURATION_STEPS) * (constants::GHOST_MAXIMUM_IDLE_DURATION - constants::GHOST_MINIMUM_IDLE_DURATION) / constants::GHOST_IDLE_DURATION_STEPS;
                break;
            }
            ...
            if (!current_platform->covers_x(next_x, constants::GHOST_WIDTH / 8.f, constants::GHOST_WIDTH / 8.f)) {
                if (auto next_platform = stage.get_platforms().find_platform_underneath({next_x, position.y}, constants::GHOST_WIDTH / 8.f, constants::GHOST_WIDTH / 8.f, constants::SEAM_SNAP_DISTANCE)) {
            ...
            if (current_climbable) {
                if (will_climb && ((!moving_right && position.x < current_climbable->get_x_pos()) || (moving_right && position.x > current_climbable->get_x_pos()))) {
                ...
                } else if ((climbing_up && !current_climbable->can_climb_up(position, constants::GHOST_WIDTH / 2.f, constants::GHOST_HEIGHT / 2.f))
                        || (!climbing_up && !current_climbable->can_climb_down(position, constants::GHOST_WIDTH / 2.f, constants::GHOST_HEIGHT / 2.f))) {
                    current_climbable.reset();
                }
            } else {
                current_climbable = stage.get_climbables().find_climbable_down_at(position, constants::GHOST_WIDTH / 2.f, constants::GHOST_WIDTH / 2.f);
                climbing_up = false;
                if (!current_climbable) {
                    current_climbable = stage.get_climbables().find_climbable_up_at(position, constants::GHOST_WIDTH / 2.f, constants::GHOST_WIDTH / 2.f);
                    climbing_up = true;
                }
                if (current_climbable) {
                    if (just_climbed) {
        ...
        case State::Climbing:
            ...
            if ((climbing_up && position.y <= current_climbable->get_upper_y_pos()) || (!climbing_up && position.y >= current_climbable->get_lower_y_pos())) {
                if (climbing_up) {
