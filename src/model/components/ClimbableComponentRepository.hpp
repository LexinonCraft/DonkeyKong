...
public:
    ...
    std::weak_ptr<Climbable> find_climbable_up_at(const sf::Vector2f &position, float h_tolerance, float v_tolerance) {
        for (auto it = begin(); it != end(); ++it) {
            auto climbable = it->second->get_entity_locked();
            if (climbable->can_climb_up(position, h_tolerance, v_tolerance)) {
                return climbable;
            }
        }
        return std::weak_ptr<Climbable>();
    }

    std::weak_ptr<Climbable> find_climbable_down_at(const sf::Vector2f &position, float h_tolerance, float v_tolerance) {
        for (auto it = begin(); it != end(); ++it) {
            auto climbable = it->second->get_entity_locked();
