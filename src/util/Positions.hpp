...
template <typename T>
sf::Vector2f compute_origin_for_anchor(T &target, AnchorPosition anchor) {
...
template <typename T>
void set_origin(T &target, AnchorPosition anchor) {
    target.setOrigin(compute_origin_for_anchor(target, anchor));
}

template <typename T>
void set_to_size(T &target, const sf::Vector2f &size) {
    sf::FloatRect bounds = target.getLocalBounds();
...
template <typename T>
void rotate_180_degrees(T &target, AnchorPosition anchor, bool rotate) {
    if (!rotate) {
        return;
    }
