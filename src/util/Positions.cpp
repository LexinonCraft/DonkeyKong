...
sf::Vector2f get_absolute_position(const sf::Vector2f &relative_position, AnchorPosition anchor) {
    ...
    return sf::Vector2f(normalized_position.x * constants::VIEW_WIDTH, (normalized_position.y - 1.f) * constants::VIEW_HEIGHT) + relative_position;
}
...
void scale(sf::Transformable &transformable, float scale) {
    transformable.setScale(transformable.getScale() * scale);
}
